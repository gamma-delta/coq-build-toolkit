...
(defn escape [str]
  (let [str (string str)]
    (reduce (fn [acc [src dst]] (string/replace-all src dst acc))
            str
            [["&" "&amp;"]
             ["<" "&lt;"]
             [">" "&gt;"]
...
(defn write
  ...
  (write! "<!-- https://github.com/gamma-delta/coq-build-toolkit -->\n")
  (process-form form formatting 0 write!)
  buf)

(defn tag*
  "Define an xml tag with the given name and key-value pairs."
  [tag & kvs]
  [tag (table ;kvs)])
